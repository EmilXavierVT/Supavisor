...
- The **environment** is a source of truth too (`package.json` scripts, app files, the directory layout, `--help` output), and a document that restates it is a **cache**: a copy of a lookup, earning its load only when the lookup is expensive. Cache what the agent cannot find by looking: the unwritten convention, the reason behind a choice, the gotcha no app confesses. Leave the one-file, one-command lookups to the environment, where they cannot go stale.
