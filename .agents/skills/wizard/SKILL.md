...
- For setup: `.env`, `.env.example`, `.env.*`, `README`, `docker-compose*`, framework app, and `.github/workflows/*` (every `secrets.*` / `vars.*` reference is a value the wizard must produce).
